...
router.route('/wholeMeals')
  .get(async (req, res) => {
    try {
      // meals and macros share a meal_id - join info
      const meals = await db.Meals.findAll();
      const macros = await db.Macros.findAll();
      // .map is a forEach that returns a new array after applying a function to an array in a loop
      const wholeMeals = meals.map((meal) => {
        const macroEntry = macros.find((macro) => macro.meal_id === meal.meal_id);
        return {
          ...meal.dataValues, // ... means return everything in the meal.dataValues object
